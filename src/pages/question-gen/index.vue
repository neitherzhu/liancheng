<template>
  <view>
    ...
    <view>
      <view v-if="!images.length">
        ...
        <up-cell-group class="text-10" v-for="(x, i) in groups" :key="x">
          <up-cell v-if="groups.length > 1">
            <template #title>
              <text class="c-primary">生成条件{{ i + 1 }}</text>
            </template>
            <template #value>
              <text class="c-primary" @click="handleCloseGroup(i)">删除</text>
            </template>
          </up-cell>
          <up-cell title="题型" :isLink="true" @click="handleOpenPicker(i)">
            <template #value>
          ...
          <up-cell title="运算位数">
            <template #value>
              <up-input v-model="x.mixCount" type="number" :border="false" inputAlign="right" />
            </template>
          </up-cell>
          <up-cell title="题目数量">
            <template #value>
              <up-input v-model="x.count" type="number" :border="false" inputAlign="right" />
            </template>
          </up-cell>
        </up-cell-group>
        <up-button v-if="groups.length < 7" size="large" class="my-2" @click="handleAddGroup">
          <up-icon name="plus" size="14" style="width:20px;height:20px"></up-icon>
          <text class="ml-1">添加题目生成条件</text>
        </up-button>
      </view>
  ...
  <up-picker :show="showPicker" @cancel="handlePickerCancel" :columns="columns" keyName="label"
    @confirm="handlePickerConfirm"></up-picker>
</template>

<script setup lang="ts">
import { genrateRandomMathQuestions, isValidNumber, Question, QuestionType } from '@/utils/biz/math'
...
const handleGen = () => {
  let errorText = ''
  let isError = groups.value.some((x: DefaultGenData) => {
  ...
  while (questionData) {
    ...
    while (!isBreak && item) {
      item.forEach((y: Question, i: number) => {
        ...
        const left = (i % 4) * questionWidth + 40
